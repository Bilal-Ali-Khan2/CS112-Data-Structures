...
public class Collage {
    ...
    public void makeCollage () {
        ...
        // WRITE YOUR CODE HERE
        Picture collageDuplicate = originalPicture; 
        for (int targetCol = 0; targetCol < (tileDimension * collageDimension); targetCol++) {
           for (int targetRow = 0; targetRow < (tileDimension * collageDimension); targetRow++) {
               int sourceCol = targetCol * collageDuplicate.width()  / (tileDimension * collageDimension);
               int sourceRow = targetRow * collageDuplicate.height() / (tileDimension * collageDimension);
               Color color = collagePicture.get(sourceCol, sourceRow);
               collageDuplicate.set(targetCol, targetRow, color);
        ...
        for(int targetCol = 0; targetCol < collagePicture.width(); targetCol++){
           ...
           int colTile = targetCol % tileDimension;
           int rowTile = targetRow % tileDimension;
           Color color = collageDuplicate.get(colTile, rowTile);
           collagePicture.set(targetCol, targetRow, color);
           }
           }
    ...
    public void colorizeTile (String component,  int collageCol, int collageRow) {
        ...
        // WRITE YOUR CODE HERE
        int tileCol = collageCol * tileDimension;
int tileRow = collageRow * tileDimension;
 
for(int targetCol = tileCol; targetCol < (tileCol + tileDimension); targetCol++)
{
for(int targetRow = tileRow; targetRow < (tileRow + tileDimension); targetRow++)
{
...
collagePicture.set(targetCol, targetRow, new Color(0, 0, b));
    ...
    public void replaceTile (String filename,  int collageCol, int collageRow) {
        ...
        Picture collageDup = new Picture(filename);
        for(int targetCol = 0; targetCol < tileDimension; targetCol++) {
        for(int targetRow = 0; targetRow < tileDimension; targetRow++) {
        int sourceCol = targetCol * collageDup.width() / tileDimension;
        int sourceRow = targetRow * collageDup.height() / tileDimension;
        Color color = collageDup.get(sourceCol, sourceRow);
        collageDup.set(targetCol, targetRow, color);
        }
        }
        ...
        int colTile = targetCol % tileDimension;
        int rowTile = targetRow % tileDimension;
        Color color = collageDup.get(colTile, rowTile);
        collagePicture.set(targetCol, targetRow, color);
        }
        }
